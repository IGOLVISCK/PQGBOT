...
#CHAVE NA OPENAI
client_ai = OpenAI(api_key="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")
...
async def gpt(ctx, *, pergunta):
    canal = bot.get_channel(1391897428038516736)
    await canal.send("Pensando... 🤔")

    try:
        response = client_ai.chat.completions.create(     #Chamada para a API do ChatGPT (via OpenAI).
            model="gpt-3.5-turbo",  # ou "gpt-4" se tiver acesso #MODELO DE IA
            messages=[
                {"role": "system", "content": "Você é um assistente útil e divertido no Discord."},
        ...
        await canal.send(resposta_texto)
